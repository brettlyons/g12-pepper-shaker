...
app.controller('PhoneController', function($scope, $location, $rootScope) {
  ...
  //const maracas = new Audio('./app/Maracas2.mp3');
  $scope.socket = io();
  $scope.shakes = 0;
  ...
  $scope.handleDeviceAccelChange = function(event) {
    ...
    if ($scope.totalAcceleration > ACCELERATION_REPORT_MIN) {
      maracas.play();
  ...
  $scope.associateImageWithUniqueId = function(uniqueId) {
    ...
    if (!$scope.imgCache[uniqueId]) {
      $scope.imgCache[uniqueId] =
        images[Math.floor(Math.random() * (images.length - 1) + 0)];
    }
  ...
  $scope.racerMover = function(data) {
    ...
    if ($scope.players[data.userId].score > SHAKES_TO_WIN) { // 100 is for 100% width on the DOM
